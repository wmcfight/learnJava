...
public final class KetamaNodeLocator {
	...
	public Node getPrimary(final String k) {
		...
		int randomNum = Math.abs(new Random().nextInt() % 4);
	...
	Node getNodeForKey(long hash) {
		...
		if(!ketamaNodes.containsKey(key)) {
			...
			//System.out.println("main key is "+ketamaNodes.firstKey()+);
//			Node mainRv = ketamaNodes.get(ketamaNodes.firstKey());
//			Node secRvNode = ketamaNodes.get(tailMap.firstKey());
			
			//System.out.print("hash value is "+hash+" main key is "+mainRv.getName() + " Sec key is "+secRvNode.getName());
			if(tailMap.isEmpty()) {
