...
public class HashAlgorithmTest {
	...
	public static void main(String[] args) {
		...
		for (String key : allKeys) {
			...
			//System.out.println("   fianl  node is "+node.getName());
		...
		for (Map.Entry<Node, Integer> entry : nodeRecord.entrySet()) {
			System.out.println("Node name :" + entry.getKey().getName() + " - Times : " + entry.getValue() + " - Percent : " + (float)entry.getValue() / EXE_TIMES * 100 + "%");
		}
		
		for(StackTraceElement[] items: Thread.currentThread().getAllStackTraces().values()){
			for(StackTraceElement item : items){
				
				//System.out.println(item. + "    " + item.getLineNumber() + "    " + item.getMethodName());
			}
		}
